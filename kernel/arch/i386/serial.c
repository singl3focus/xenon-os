...
void serial_configure_baud_rate(unsigned short com, unsigned short divisor) {
    ...
    outb(SERIAL_DATA_PORT(com), (divisor >> 8) & 0x00FF); // Старший байт
    outb(SERIAL_DATA_PORT(com), divisor & 0x00FF);        // Младший байт
}
...
void serial_init() {
    serial_configure_baud_rate(SERIAL_COM1_BASE, 1); // 38400 бод (115200 / 3)
